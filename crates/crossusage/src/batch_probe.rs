//! Batch probe timeout — `std::thread` + channel (see crossusage-cli `batch_probe`).
...
pub fn run_probe_with_timeout(
    ...
    loop {
        ...
        if now >= deadline {
            eprintln!(
                "crossusage: probe timed out after {timeout_sec}s for provider `{plugin_id}` \
                 (set CROSSUSAGE_PROBE_TIMEOUT_SEC or e.g. `crossusage list cursor`.)"
            );
