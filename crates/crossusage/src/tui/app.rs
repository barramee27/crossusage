//! Ratatui main loop: layout, input, staggered probes.
...
use crossusage_core::plugin_engine::manifest::LoadedPlugin;
use crossusage_core::plugin_engine::runtime::{self, PluginOutput};
...
pub fn run(
    ...
    if !stdin().is_terminal() || !stdout().is_terminal() {
        anyhow::bail!(
            "The dashboard needs an interactive terminal (stdin and stdout must be TTYs). \
             Open a real terminal window, or use `crossusage-cli list` / `crossusage-cli probe` for non-interactive output."
        );
    }

    platform::ignore_sigtstp();
    eprintln!(
        "crossusage-cli: starting dashboard — first probe can take a while per provider. \
         Quit with q (confirm) or close this message and wait. Do not use Ctrl+Z (suspend)."
    );
    if platform::parent_process_is_cargo() {
        eprintln!(
            "crossusage-cli: you are running under `cargo run` (parent: cargo). \
             If bash shows `[1]+ Stopped cargo run …` even though you did not press Ctrl+Z, \
             build once (`cargo build -p crossusage-cli`) and run `./target/debug/crossusage-cli` — \
             that avoids the extra cargo process in the shell job."
        );
    }

    let mut initial = Vec::with_capacity(selected_indices.len());
    for (n, &idx) in selected_indices.iter().enumerate() {
        eprintln!(
            "crossusage-cli: probing {}/{}…",
            n + 1,
            selected_indices.len()
        );
        let out = runtime::run_probe(&plugins[idx], &app_data, &version);
        initial.push(out);
        // Plugins / QuickJS may reset signal masks; keep job-control stops ignored.
        platform::ignore_sigtstp();
    }

    let mut state = AppState::new(config_path, config.clone(), initial);
    ...
    if state.config.mouse {
        execute!(term_out, DisableMouseCapture)?;
    }
    execute!(term_out, LeaveAlternateScreen, Show)?;
    disable_raw_mode()?;
    let _ = result;
    Ok(())
}
...
fn run_inner(
    ...
    loop {
        ...
        terminal.draw(|f| {
            ...
            if state.quit_open {
                draw_quit_modal(f, size, &theme);
            } else if state.help_open {
                draw_help_modal(f, size, &theme);
            }
        })?;

        let timeout = state.ui_tick().saturating_sub(tick.elapsed());
                ...
                Event::Key(key) if key.kind == KeyEventKind::Press => {
                    ...
                    if state.help_open {
                        match key.code {
                            ...
                            KeyCode::Char('q') => state.quit_open = true,
                            _ => {}
                        }
                        continue;
                    }
                    ...
                    {
                        state.quit_open = true;
                        continue;
                    }
                    match key.code {
                        KeyCode::Char('q') | KeyCode::Char('Q') => state.quit_open = true,
                        ...
                        KeyCode::Char('p') | KeyCode::Char('P') => {
                            ...
                            state.status_msg =
                                Some(format!("Low-power: {}", state.config.low_power_mode));
                        }
                        ...
                        KeyCode::Char('=') | KeyCode::Char('0') => {
                            *layout_ratios = normalize_ratios([22, 48, 30]);
                            state.status_msg = Some("Pane layout reset".to_string());
                        }
                        _ => {}
                    }
                }
        ...
        if tick.elapsed() >= state.ui_tick() {
            *tick = Instant::now();
            ...
            maybe_schedule_probe(state, plugins, selected_indices, app_data, version, tx);
        }
    }
}
...
fn maybe_schedule_probe(
    ...
    if now.duration_since(state.last_full_probe) >= full_every {
        spawn_full_probe(state, plugins, selected_indices, app_data, version, tx);
...
fn draw_header(f: &mut Frame, area: Rect, state: &AppState, theme: &Theme) {
    ...
    let line = format!(
        " CrossUsage  |  {}  |  CPU {:4.1}%  MEM {}/{} MiB  |  est. spend: {} ",
...
fn format_plugin_lines(o: &PluginOutput) -> String {
    use crossusage_core::plugin_engine::runtime::{MetricLine, ProgressFormat};
...
fn draw_status_bar(f: &mut Frame, area: Rect, state: &AppState, theme: &Theme) {
    ...
    let line = format!(
        "crossusage-cli | {low}{stale} | {probe}| probe {}s | {extra}↑↓jk r refresh p low-power ? help q quit",
...
fn draw_help_modal(f: &mut Frame, r: Rect, theme: &Theme) {
    ...
    let help = Paragraph::new(
        "q        Quit (asks confirm)\n\
         ...
         = 0      Reset pane widths (after mouse resize)\n\
...
fn draw_quit_modal(f: &mut Frame, r: Rect, theme: &Theme) {
    let block = Block::default()
        .title(" Quit ")
        .borders(Borders::ALL)
        .border_style(Style::default().fg(theme.warn));
    let area = centered_rect(50, 20, r);
    f.render_widget(Clear, area);
    f.render_widget(block.clone(), area);
    let inner = block.inner(area);
    let p = Paragraph::new("Quit CrossUsage?  (y/n)")
        .style(Style::default().fg(theme.fg))
