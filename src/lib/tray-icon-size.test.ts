...
describe("tray-icon-size", () => {
  it("resolveTrayIconBasePt is 18 on macOS, 34 on Windows and Linux desktop", () => {
    expect(resolveTrayIconBasePt("Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7)")).toBe(18)
    expect(resolveTrayIconBasePt("Mozilla/5.0 (Windows NT 10.0; Win64; x64)")).toBe(34)
    expect(resolveTrayIconBasePt("Mozilla/5.0 (X11; Linux x86_64)")).toBe(34)
  ...
  it("getTrayIconSizePx is 34/68 at 1x/2x with Windows UA", () => {
    vi.stubGlobal("navigator", { userAgent: "Mozilla/5.0 (Windows NT 10.0; Win64; x64)" })
    try {
      expect(getTrayIconSizePx(1)).toBe(34)
      expect(getTrayIconSizePx(2)).toBe(68)
    } finally {
      vi.unstubAllGlobals()
    }
  })
