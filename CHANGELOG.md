...
- Merged **[OpenUsage v0.6.14](https://github.com/robinebers/openusage/releases/tag/v0.6.14)** into this fork (clickable provider rows and session/weekly labels, Claude PromoClock peak/off-peak, cmd-arrow tab navigation, Gemini session-expired fix, Antigravity plan label preference, macOS panel/tray positioning, host API hardening, WebKit scheduling guard on older macOS, dependency bumps).
